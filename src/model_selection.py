...
# Tensorboard ki help se hamare paas pure model ki history rhegi ki har run ke sath kya improvement aye model mai aur kya changes karne ki hame zarurat hai
...
class ModelSelection:
    ...
    def load_data(self):
        ...
        # Rename columns: Remove special characters and replace spaces with underscores ( This has been done for LGBM )
            df.columns = df.columns.str.replace(r'[^A-Za-z0-9_]', '', regex=True)
        
            df_sample = df.sample(frac=0.1, random_state=42)  # Sample 10% of data
        ...
        try:
            ...
            for idx, (name, model) in enumerate(self.models.items()):
                try:
                    ...
                    continue  # Skip to next model if one fails
    ...
    def run(self):
        try:
            ...
            logger.error(f"Error in the pipeline: {str(e)}")  # Log the exact error message
            raise CustomException(f"Error in the pipeline: {str(e)}", sys)
